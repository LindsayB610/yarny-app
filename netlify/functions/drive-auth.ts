...
import { parseSessionFromEvent, createErrorResponse } from "./types";
...
export const handler: NetlifyFunctionHandler = async (
  ...
  if (!clientIdValidation.valid) {
    ...
    return createErrorResponse(500, `Invalid Client ID format: ${clientIdValidation.error}`);
  }
  ...
  if (!clientSecretValidation.valid) {
    ...
    return createErrorResponse(
      500,
      `Invalid Client Secret format: ${clientSecretValidation.error}`
    );
  }
