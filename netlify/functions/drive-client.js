...
async function getTokens(email) {
    try {
        const store = (0, blobs_1.getStore)(getStoreOptions());
        let allTokens = {};
        try {
        ...
        console.log("getTokens - error:", error instanceof Error ? error.message : String(error));
        return null;
    }
}
async function saveTokens(email, tokens) {
    try {
        ...
        console.error("saveTokens - error saving:", error instanceof Error ? error.message : String(error));
        throw error;
    }
}
...
async function getAuthenticatedDriveClient(email) {
    ...
    if (!expiryTime || now >= expiryTime - 300000) {
        ...
        try {
            ...
            if (errorMessage.includes("invalid_grant") ||
                ...
                console.error("Refresh token expired or revoked. Clearing stored tokens for:", email);
                try {
                ...
                throw new Error("Refresh token expired or revoked. Please re-authorize Drive access.");
            }
    ...
    const driveClient = googleapis_1.google.drive({ version: "v3", auth: oauth2Client });
